...
- `running.rs` — `RunningProcess` spawns through `command-group` so the whole process **group** can be tree-killed. stdout/stderr are read on threads into an mpsc channel and drained non-blockingly by `poll()` each frame. Stop is graceful: SIGINT (Ctrl+C — build tools like Gradle relay it to their child and keep its shutdown logs flowing, unlike SIGTERM) → 10 s grace → SIGKILL on the group; pressing Stop again during the grace escalates to SIGKILL at once. `Drop` force-kills the group, so closing the app also stops its servers — which is why the release profile keeps unwinding (**no `panic = "abort"`**): abort would skip `Drop` and orphan child processes. A `RunningProcess` is either **Owned** (spawned here, with live log pipes) or **Adopted** (recovered from a previous session by PID — stop/restart work, but there are no live logs); the public API is identical, so the UI is agnostic.
- Status: a server with a port starts as `Starting` and becomes `Running` when `poll()` sees the port taken (a throttled bind probe, 127.0.0.1 only); no port means `Running` at once. `stop()` sets `Stopping` for the grace window. Neither is terminal.
- `instance_lock.rs` — `campfire.lock` (PID + start time) in the data dir; a second launch that finds the first alive shows a dialog and exits. Two instances would otherwise share `running.json`, and the second would adopt the first's servers and kill them on close.
- `log_buffer.rs` — a byte-capped 5 MiB ring buffer; `total_pushed()` is monotonic so the log view can count unseen lines.
...
### Workspaces (multi-view logs) — `src/ui/workspaces/`
- Tabbed **workspaces** (≤100), each an `egui_tiles::Tree<String>` split of ≤4 log panes (pane payload = server id). Log DATA stays in the shared `running` map; each workspace holds only layout plus per-server `LogView` state, so one server can be open in several workspaces independently.
- **Opening is drag-to-place**: dragging a sidebar card over the dock previews the drop (nearest-edge half of the hovered pane; `drop.rs` zone math + tree surgery, both unit-tested) and releasing splits there. An already-open project is focused instead. The card context menu's "Open log" is the non-drag fallback; a plain click shows the project in the focused pane. Sidebar cards use stable project IDs and stay stationary during dragging. A release inside the sidebar reorders at the insertion marker; a release in the dock never reorders the list.
- egui_tiles has **no external drag-in** — the drop overlay/insert is custom (`tiles.rect()` is only valid after `Tree::ui` ran that frame). Internal pane drags (title strip → `UiResponse::DragStarted`) and split resizes are egui_tiles built-ins that mutate the tree during rendering — which is why `Workspaces` is `&mut` UI state. Workspaces are deliberately **session-only** (every launch starts from one fresh empty workspace); a persistence layer existed briefly and was removed as not worth the state.
- Every log view instance takes an id **salt** (`log_view::show(ui, salt, ..)`) keyed by `(workspace id, server id)` — multiple panes would otherwise clash on the fixed panel/scroll ids and egui paints red id-clash warnings.
...
- **Section-block layout**: the UI is white rounded blocks (`block_frame`, one shared radius) floating on a cool light-grey canvas (`CANVAS_FILL`). Sections are separated by surface contrast, **not** divider lines — panels set `.show_separator_line(false)` and carry the inter-block gaps as margins via `canvas_frame(margin)`. When adding a section, wrap it in `block_frame` rather than reaching for `ui.separator()`. Frame helpers: `block_frame` (top-level section) / `card_frame` (list item) / `modal_frame` (dialog) / `inset_frame` (code block).
...
- Rows that pair a variable-width name with fixed trailing items (sidebar card, pane header) lay the trailing items out first (right-to-left) and give the name a `Label::truncate()` in the remaining space, so a narrow width elides the name instead of overlapping.
- Sidebar project cards are **drag-to-reorder** (order persists via the normal config save, so the `servers` list order IS the on-disk order). Detect the drag with `response.dragged()` + `Response::dnd_set_drag_payload`, **not** `is_being_dragged(Id::new(..))`: `Frame::Prepared::allocate_space().interact()` reuses an auto-generated id, so a hand-built id never matches, the payload is silently never set, and the drag compiles clean but does nothing at runtime. `Sense::click_and_drag()` keeps click-to-select working alongside the drag.
...
- `system.rs` — OS hand-offs (open a URL / reveal a folder via `open` / `cmd /C start` / `xdg-open`). Called directly from render code: they touch no app state, so they don't go through `Action`.
