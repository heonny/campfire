...
- **Opening is drag-to-place**: dragging a sidebar card over the dock previews the drop (nearest-edge half of the hovered pane; `drop.rs` zone math + tree surgery, both unit-tested) and releasing splits there. The card context menu's "Open log" is the non-drag fallback; a plain click only focuses. egui_dnd reports a reorder even when released far outside the list, so `server_list` swallows `Reorder` when the release lands in the dock rect (taken from the previous frame).
